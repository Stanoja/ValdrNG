...
describe('ValdrNgService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        {
          provide: BaseValidatorFactory,
          useClass: SizeValidatorFactory,
          multi: true,
        },
        {
          provide: BaseValidatorFactory,
          useClass: RequiredValidatorFactory,
          multi: true,
        },
      ],
    });
  ...
  it('it should add valdr validators', () => {
    ...
    // then
    expect((service as any).validatorsPerField['testConstraint']).toBe(
      validator
    );
  });
  ...
  describe('createValidators', () => {
    ...
    it('should throw error if the type is not present', () => {
      // given / when / then
      expect(() => service.createFormGroupControls({}, 'SomeModel')).toThrow(
        new Error("No constraints provided for type 'SomeModel'.")
      );
    });
    ...
    it('should create form group controls for fields which are not present in the constraints', () => {
      ...
      expect(formGroup).toEqual(
        jasmine.objectContaining({
          firstName: jasmine.arrayContaining([
            model.firstName,
            jasmine.any(Array),
          ]),
    ...
    it('should create additional constraints for fields which are not present in the model', () => {
      ...
      // when
      const formGroup = service.createFormGroupControls(
        model,
        'Person',
        additionalControls
      );
      ...
      // then
      expect(formGroup.lastName).toEqual(
        jasmine.arrayContaining(['Sst', [validatorFn]])
      );
    });
  });
  ...
  describe('getValidatorsForField', () => {
    ...
    it('should throw error if the type is not present', () => {
      // given / when / then
      expect(() =>
        service.getValidatorsForField('SomeModel', 'SomeField')
    ...
    it('should throw error if the field is not present', () => {
      // given / when / then
      expect(() =>
        service.getValidatorsForField('Person', 'SomeField')
    ...
    it('should warn if no validators are available for constraint', () => {
      ...
      expect(console.warn).toHaveBeenCalledOnceWith(
        "No validator found for constraint 'required'."
      );
